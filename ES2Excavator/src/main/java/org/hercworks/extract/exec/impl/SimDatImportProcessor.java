...
import org.hercworks.extract.util.impl.SimFileImportMatcher;
...
public class SimDatImportProcessor extends GenericJsonProcessor{
	
	private SimFileMatcher match = new SimFileImportMatcher();
	...
	public boolean filterFile(FileItem file) {
		if(match.matchFile(file.getName())) {
				...
				for(HercLUT herc : HercLUT.values()) {
					if(file.getName().toLowerCase().contains(herc.getAbbrevDat().toLowerCase())) {
						if(file.getName().toLowerCase().contains(FileType.DMG.name().toLowerCase())) {
							importJson(file.getName(), new HercDamageFileTransformer(), HercSimDamage.class, new HercSimDmgDTOServiceImpl(), HercDmgDTO.class);
//							processed = true;
							break;
						}
...
//							processed = true;
							...
							importJson(file.getName(), new GunLayoutTransformer(), GunLayout.class, new GunLayoutDTOServiceImpl(), GunLayoutDTO.class);
							break;
						}
						else if(file.getName().toLowerCase().contains(FileType.PDG.name().toLowerCase())) {
							importJson(file.getName(), new PaperDiagramGraphTransformer(), PaperDollGraphic.class, new PaperDollDTOServiceImpl(), PaperDollDTO.class);
							break;
						}
					}
				}
				...
				case BASE_COL:
					break;
				case BEAM:
					importJson(file.getName(), new BeamDatFileTransformer(), BeamData.class, new BeamDatDTOServiceImpl(), BeamDatDTO.class);
					break;
				case BFORMS:
					break;
				case BULLETS:
					importJson(file.getName(), new MissileDatFileTransformer(), MissileDatFile.class, new MissileDatDTOServiceImpl(), MissileDatDTO.class);
					break;
				case COLORS:
					break;
				case DEBRIS:
				...
				case MFORMS:
					break;
				case PROJECTILE:
					importJson(file.getName(), new ProjectileDataTransformer(), ProjectileData.class, new ProjectileDatDTOServiceImpl(), ProjectileDataDTO.class);
					break;
				case ROCKETS:
					importJson(file.getName(), new MissileDatFileTransformer(), MissileDatFile.class, new MissileDatDTOServiceImpl(), MissileDatDTO.class);
					break;
